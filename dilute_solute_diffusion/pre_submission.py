...
def run(mpfile, hosts=None):
    ...
    for idx, host in enumerate(host_info):
        ...
        if str(host)=='Fe':
            ...
            ms_t = {"M Squared": [0.97318225,0.96864964,0.96334225,0.95981209,0.95531076,0.95004009,0.944784,0.93779856,0.93006736,0.92217609,0.91431844,0.90497169,0.89548369,0.8836,0.87254281,0.86081284,0.84842521,0.83503044,0.82029249,0.80568576,0.78872161,0.77369616,0.75550864,0.73685056,0.71605444,0.69488896,0.67026969,0.64384576,0.61528336,0.58308496,0.54848836,0.51108201,0.46949904,0.42068196,0.36156169,0.30780304,0.24482704,0.16630084], "T": [250.56,271.44,292.32,313.2,334.08,354.96,375.84,396.72,417.6,438.48,459.36,480.24,501.12,522,542.88,563.76,584.64,605.52,626.4,647.28,668.16,689.04,709.92,730.8,751.68,772.56,793.44,814.32,835.2,856.08,876.96,897.84,918.72,939.6,960.48,981.36,1002.24,1023.12]}
            df_ms_t = DataFrame(ms_t)
            df_ms_t.columns = ['msquared','T (K)']
            mpfile.add_data_table(mpid, df_ms_t, 'MS_T')

        elif hdata[mpid]['Host crystal structure']=='HCP':
           df_D0_Q = df[['Solute element name', 'Solute D0 c-axis [cm^2/s]', 'Solute Q c-axis [eV]']]
           df_D0_Q.columns = ['element', 'D0 [cm2/s]', 'Q [eV]']
           df_D0_Q_basal = df[['Solute element name', 'Solute D0 basal [cm^2/s]', 'Solute Q basal [eV]']]
           df_D0_Q_basal.columns = ['element', 'D0 [cm2/s]', 'Q [eV]']
           mpfile.add_data_table(mpid, df_D0_Q_basal, 'D0_Q_basal')
